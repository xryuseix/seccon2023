...
// ここから下はTypeScriptの型宣言で、HTTP リクエストボディにpasswordがあることを示しています
const Chall3ReqBodySchema = z.object({
  password: z.string(),
});
export type Chall3ReqBody = z.infer<typeof Chall3ReqBodySchema>;

export function chall3(req: Chall3ReqBody): ChallRes {
  // ユーザが変な値を送ってきていないか確認しているだけなので気にしなくてOK
  const parseResult = Chall3ReqBodySchema.safeParse(req);
